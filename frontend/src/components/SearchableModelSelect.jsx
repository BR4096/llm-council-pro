...
export default function SearchableModelSelect({
  ...
  const providerOrder = [
    'OpenAI (Direct)', 'Anthropic (Direct)', 'Google (Direct)', 'Perplexity (Direct)', 'Mistral (Direct)', 'DeepSeek (Direct)',
    'Groq (Direct)', 'GLM (Direct)', 'Kimi (Direct)',
    'OpenRouter (Cloud)',
    'Local (Ollama)'
  ];
  ...
  // Final fallback: show raw model ID with hint if model not found anywhere
  // This handles cases where provider is disabled or model is deprecated
  if (!selectedOption && value) {
    // Extract short name from model ID for cleaner display
    // Strip provider prefix only (first colon), preserving model:tag format for Ollama
    // e.g., "ollama:llama3.1:8b" -> "llama3.1:8b", "openai:gpt-4.1" -> "gpt-4.1"
    let shortName = value;
    if (value.includes('/')) {
    ...
    selectedOption = {
      value: value,
      label: `${shortName} (enable provider)`,
    };
  }
