...
import perplexityLogo from '../assets/icons/perplexity.svg';
...
import glmLogo from '../assets/icons/glm.svg';
...
const PROVIDER_CONFIG = {
    ...
    perplexity: { color: '#1fb8cd', label: 'Perplexity', logo: perplexityLogo },
    ...
    glm: { color: '#3b82f6', label: 'GLM', logo: glmLogo },
    kimi: { color: '#1e3a8a', label: 'Kimi', logo: kimiLogo },
...
const getProviderInfo = (modelId) => {
    ...
    if (id.startsWith('perplexity:')) return PROVIDER_CONFIG.perplexity;
    if (id.startsWith('glm:')) return PROVIDER_CONFIG.glm;
    if (id.startsWith('kimi:')) return PROVIDER_CONFIG.kimi;
...
const getModelDisplayName = (modelId) => {
    ...
    if (modelId.startsWith('placeholder')) return 'Member';
...
export default function CouncilGrid({
    ...
    return (
        <div className={gridClass}>
            ...
            {displayModels.map((modelId, index) => {
                ...
                return (
                    <div
                        ...
                        style={{ '--provider-color': info.color, opacity: isFollowUpMode ? 0.35 : 1, transition: 'opacity 0.5s' }}
                        onMouseEnter={(e) => handleMouseEnter(e, modelId)}
                        onMouseMove={handleMouseMove}
                        onMouseLeave={handleMouseLeave}
                    >
                        <div className="role-badge member">
                            {characterNames?.[index] || `Member #${index + 1}`}
                        </div>
            ...
            {/* Chairman Card - Always static, shows provider icon and model name */}
            <div
                className="council-card chairman ready"
                style={{ '--provider-color': chairmanInfo ? chairmanInfo.color : '#94a3b8' }}
                onMouseEnter={(e) => handleMouseEnter(e, chairman || 'Chairman')}
                onMouseMove={handleMouseMove}
                onMouseLeave={handleMouseLeave}
            >
                <div className="role-badge chairman">
                    {chairmanCharacterName || 'Chairman'}
                </div>
                <div className="council-avatar">
                    {chairmanInfo && chairmanInfo.logo ? (
                        ...
                        <span className="avatar-icon">{chairmanInfo ? chairmanInfo.icon : '✨'}</span>
                    )}
                </div>
                <div className="council-info">
                    <span className="model-name">
                    ...
                    <span className="provider-label">
                        {chairmanInfo ? chairmanInfo.label : 'Final Verdict'}
                    </span>
                </div>
            </div>
        </div>
