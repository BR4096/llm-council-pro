...
def _search_duckduckgo(query: str, max_results: int = 6, full_content_results: int = 3) -> str:
    """
    ...
    for idx, url in urls_to_fetch:
        ...
        if firecrawl_key:
            content = _fetch_with_firecrawl_sync(url, firecrawl_key, timeout=min(remaining, 25.0))
        else:
...
def _fetch_with_firecrawl_sync(url: str, api_key: str, timeout: float = 25.0) -> Optional[str]:
    """Fetch article content via Firecrawl API (sync version for DuckDuckGo)."""
    try:
        client = get_sync_client()
        ...
        logger.warning(f"Timeout while fetching content via Firecrawl for {url}")
        return None
    except Exception as e:
        logger.warning(f"Failed to fetch content via Firecrawl for {url}: {e}")
        return None
...
async def _fetch_with_firecrawl(url: str, api_key: str, timeout: float = 25.0) -> Optional[str]:
    """Fetch article content via Firecrawl API (async)."""
    try:
        client = get_async_client()
...
async def _search_tavily(query: str, max_results: int = 6) -> str:
    """
...
async def _search_brave(query: str, max_results: int = 6, full_content_results: int = 3) -> str:
    """
    ...
    try:
        ...
        for idx, url in urls_to_fetch:
            ...
            firecrawl_key = os.environ.get("FIRECRAWL_API_KEY")
            if firecrawl_key:
                content = await _fetch_with_firecrawl(url, firecrawl_key, timeout=min(remaining, 25.0))
            else:
